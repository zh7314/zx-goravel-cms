...
export default {
	...
	methods: {
		...
		async submit() {
			...
			this.admin.admin_group_ids = checkedKeys
			//防止重置密码
			this.admin.password = null

			var res = await this.$API.system.admin.save.post(this.admin);
			// console.log(res)
			if (res.code == 200) {
				// console.log(res)
				this.visible = false;
		...
		async getMenu() {
			...
			this.menu.checked = this.admin.admin_group_ids ?? []
			this.$nextTick(() => {
